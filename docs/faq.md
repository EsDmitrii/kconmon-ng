...
tracing and the Console. The project README carries an honest comparison
table against kconmon, goldpinger and kubenurse.
...
No. It is off by default, and everything it shows comes from Prometheus and
the controller API — three Grafana dashboards ship with the project, the
...
It measures **node-to-node transport** — agents probe each other pod-IP to
pod-IP, one series per ordered node pair per protocol. That is the layer a
CNI bug, a conntrack overflow or a fabric problem lives in. It does not trace
your application's connections: for the path through ingress, Service and
API server, or an HTTP-level pod graph, the README's comparison section
...
of recording failures, and stale per-pair gauges are reset when topology
changes rather than left as ghost readings.

### How do I change checker settings without a rollout?

The binaries watch their config file and hot-reload it — an edited ConfigMap
propagates with no restart. Note that `helm upgrade` still rolls the pods on
a config change, deliberately: the workloads carry a checksum annotation so a
values edit cannot leave old and new config running side by side. Either way
the config is parsed strictly: unknown keys or invalid settings fail startup,
and on hot-reload an invalid config is rejected while the previous one stays
active.
...
and pairs grow as N×(N−1) — about 690k series at 100 nodes. The full
arithmetic is in
...
In order of preference: `agent.metrics.detail: counters-only` drops the
per-pair histograms (~70 → ~10 series per pair) while every pair alert keeps
firing; `zone-only` keeps only the Z² zone-pair plane; disabling a checker
removes its families. All are scrape-time relabelings on the ServiceMonitor —
see [Full mesh and its limits](concepts/mesh-and-planes.md#full-mesh-and-its-limits).
...
A deliberate, bounded trade-off: the port is never exposed outside the
cluster, and the optional NetworkPolicy pins it further. Outside that
boundary the same channel is disqualifying — anyone who can reach it can
register agents, receive the full peer list and steer the fleet's probes —
which is why external agents are planned around a **separate** authenticated
TLS gateway rather than around "just expose the port". Details in
