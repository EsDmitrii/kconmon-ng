// Package kubectx captures Kubernetes events into the Console's k8s_events table so the Investigate
// timeline can put "the kubelet evicted this pod" and "this node went NotReady" next to the loss
// and latency the fleet measured; it is the ONLY part of the Console that talks to the Kubernetes
// apiserver.
package kubectx
...
const (
	// messageMaxLen caps a stored event message at 1 KiB, INCLUDING the truncation marker.
	messageMaxLen = 1024
	// truncationMarker makes a cut message read as cut; without it an operator has no way to tell a
	// truncated message from one that genuinely ended there.
	truncationMarker = "..."

	// topologyRefreshInterval bounds how often the node-name set is re-read from the controller; a
	// read per event would turn an event storm into a controller DoS.
	topologyRefreshInterval = 30 * time.Second

	// The floor exists so a stream that keeps closing cannot become a hot relist loop against the
	// apiserver.
	defaultMinBackoff = 1 * time.Second
	defaultMaxBackoff = 30 * time.Second

	// defaultResyncInterval backs up config validation; the reader is only ever spawned with a
	// validated config.
...
// There are exactly TWO, and the split is forced by Kubernetes; there is no namespace that means
// "node events".
type stream struct {
	name          string
	namespace     string
...
type Reader struct {
	...
	// namespace is the RESOLVED pod-event namespace (config, then POD_NAMESPACE, then "default"),
	...
	// nodes is the cached topology node-name set and nodesAt is when it was last ATTEMPTED (success or
	// failure alike, so a controller outage cannot turn one failed read into a per-event retry).
	mu      sync.Mutex
...
// It never fails and never touches the network: the clientset is already built by the caller.
func New(
...
// watchLoop keeps one stream alive for the life of ctx.
...
// listAndWatch performs one LIST, then WATCHes from the list's resourceVersion until the watch
// ends; every relisted event the database already holds costs one conflicting INSERT and one
// duplicate counter increment.
	...
	for {
		...
		case ev, ok := <-w.ResultChan():
			...
			case watch.Deleted, watch.Bookmark:
				// A deleted Event did not un-happen, and the capture is a historical record.
			}
		}
	}
}

// handle applies the filter to one event and, when it survives; it is not a filter decision: it
// means the apiserver ignored the field selector and the other stream owns this event.
func (r *Reader) handle(ctx context.Context, ev *corev1.Event, s stream) {
	...
	case err != nil:
		...
		// Keyed by reason, not by object: one broken emitter produces one log line a minute instead of
		// one per event.
...
func (r *Reader) keep(ctx context.Context, ev *corev1.Event, s stream) bool {
	...
	// Pod events are kept unconditionally WITHIN the namespace; the check is defensive: the watch is
	// already namespace-scoped.
	return involvedNamespace(ev) == r.namespace
}

// nodeInTopology reports whether the controller's topology currently vouches for name.
...
// involvedNamespace returns the namespace of the object the event is ABOUT, falling back to the
...
// logLimiter admits one log line per key per logRateLimit; the keys are derived from
// cluster-controlled strings (a k8s event Reason).
...
func (l *logLimiter) allow(key string) bool {
	...
	if len(l.last) >= logReasonsMax {
		// Drop everything rather than evict cleverly: the map is a rate limiter.
		clear(l.last)
	}
	l.last[key] = now
	return true
