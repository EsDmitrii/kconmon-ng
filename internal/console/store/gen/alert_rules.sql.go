...
const updateAlertRuleSyncStatusIfUnchanged = `-- name: UpdateAlertRuleSyncStatusIfUnchanged :execrows
UPDATE alert_rules
SET sync_status = $2, sync_message = $3, last_synced_at = $4
WHERE id = $1 AND updated_at = $5
`

type UpdateAlertRuleSyncStatusIfUnchangedParams struct {
	ID           pgtype.UUID
	SyncStatus   string
	SyncMessage  string
...
func (q *Queries) UpdateAlertRuleSyncStatusIfUnchanged(ctx context.Context, arg UpdateAlertRuleSyncStatusIfUnchangedParams) (int64, error) {
	result, err := q.db.Exec(ctx, updateAlertRuleSyncStatusIfUnchanged,
		arg.ID,
		arg.SyncStatus,
		arg.SyncMessage,
		arg.LastSyncedAt,
