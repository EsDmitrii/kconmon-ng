...
WHERE ($1::text = 'user'
   ...
   OR (subject_kind = 'group' AND subject_id = ANY($3::text[]))
ORDER BY role_name
`
...
func (q *Queries) ListBindingsForSubject(ctx context.Context, arg ListBindingsForSubjectParams) ([]RoleBinding, error) {
	rows, err := q.db.Query(ctx, listBindingsForSubject, arg.CallerKind, arg.UserID, arg.Groups)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) RehashUserPassword(ctx context.Context, arg RehashUserPasswordParams) (int64, error) {
	result, err := q.db.Exec(ctx, rehashUserPassword, arg.NewHash, arg.ID, arg.OldHash)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
...
UPDATE users
SET disabled = $1,
    session_epoch = session_epoch + CASE WHEN $1 THEN 1 ELSE 0 END,
    updated_at = now()
WHERE id = $2
`
...
func (q *Queries) SetUserDisabled(ctx context.Context, arg SetUserDisabledParams) (int64, error) {
	result, err := q.db.Exec(ctx, setUserDisabled, arg.Disabled, arg.ID)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
