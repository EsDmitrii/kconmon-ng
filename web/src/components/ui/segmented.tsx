...
/* Segmented: a real radiogroup with a physically sliding thumb. */
...
export function Segmented<T extends string>({
  ...
  /*
   * y/h ride along with x/w; measuring the active option's own box in both axes costs nothing on a
   * single-row track (where offsetTop is simply the track's padding) and is the only thing that
   * makes a wrapped one honest.
   */
  ...
  return (
    <div
      ...
      className={cn(
        "relative inline-flex max-w-full shrink-0 flex-wrap items-center rounded-md bg-surface-2 p-1",
        className,
      )}
    >
      {thumb ? (
      ...
      {options.map((opt, i) => {
        ...
        return (
          <button
            ...
            className={cn(
              /* whitespace-nowrap: a two-word option ("Zone pair") is ONE label and never breaks. */
